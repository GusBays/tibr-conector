...
export async function resourceRoutesKoa(router: Router): Promise<void> {
    const { index, show, update, destroy, sync, fetch, getImage, createImage, deleteImage, webhook } = resourceHandler()
    ...
    router.post(`${path}/:id/fetch`, auth, fetch)
    router.post(`${path}/:id/images`, auth, koaBody({ multipart: true }), createImage)
...
function resourceHandler() {
    ...
    const destroy = async (ctx: Context): Promise<void> => {
        await service.delete(KoaHelper.extractParams<ResourceFilter>(ctx))
        KoaResponse.noContent(ctx)
    }

    const fetch = async (ctx: Context): Promise<void> => {
        const resource = await service.fetch(KoaHelper.extractParams<ResourceFilter>(ctx))
        KoaResponse.success(ctx, resource)
    }
    ...
    return { index, show, update, destroy, sync, fetch, getImage, createImage, deleteImage, webhook }
}
